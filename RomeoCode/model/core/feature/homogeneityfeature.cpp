...
HomogeneityFeature::HomogeneityFeature(int idFeat):SecondOrderFeature(Utils::FEATURE_HOMOGENEITY,idFeat){

}

HomogeneityFeature::HomogeneityFeature(const QStringList& parameters, int idFeat)
    :SecondOrderFeature(Utils::FEATURE_HOMOGENEITY,parameters, idFeat){
}

InternalData2D* HomogeneityFeature::singleChannelEXecution2D(InternalData2D* channel){
    ...
    for (int y=halfIntWindow; y<ySize; y++){
        ...
        for (int x=halfIntWindow; x<xSize; x++){
            ...
            //calculate homogeneity pixel in "index" position
            float ct = 0;
            for (int i=0;i<size;i++){
                for(int j=0;j<size;j++) {
                    ct = ct+(GLCM->GetFrequency(size*j+i)*(1/(1+abs(i-j))));
                }
            }
            if (ct>max)
                max=ct;
...
InternalData3D* HomogeneityFeature::singleChannelEXecution3D(InternalData3D* channel)
{
    ...
    for (int z=halfIntWindow; z<zSize; z++){
        ...
        for (int y=halfIntWindow; y<ySize; y++){
            ...
            for (int x=halfIntWindow; x<xSize; x++){
                ...
                //calculate homogeneity voxel in "index" position
                float ct = 0;
...
HomogeneityFeature::~HomogeneityFeature()
{
